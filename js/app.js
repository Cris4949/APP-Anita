...
function cargarGuardado(){
  try{
    const guardado = JSON.parse(localStorage.getItem('piramideOlfativa_anita') || '{}');
...
function guardar(){
  ...
  try{ localStorage.setItem('piramideOlfativa_anita', JSON.stringify(data)); }catch(e){
...
function actualizarPreview(){
  ...
  const negocio = $('businessName').value.trim() || 'Perfumería Anita';
...
// ---------- Inicio ----------
$('pv-watermark-logo').src = 'assets/logo-perfumeria.jpg';
...
if(!$('businessName').value.trim()){
  $('businessName').value = 'Perfumería Anita';
}
